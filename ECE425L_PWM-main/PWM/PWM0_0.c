...
{
	// Return from the function if the specified duty cycle is greater than 
  ...
  if (duty_cycle >= period_constant) return;
	...
	// The 0x4 value is derived from Table 23-5 in the TMC4123G Microcontroller Datasheet
	...
	// configuration by clearing the ENABLE bit (Bit 0) in the PMW0CTL register
	PWM0->_0_CTL &= ~0x01;
	...
	// in the PWM0CMPA register. This determines the number of clock
	...
	// Enable the PWM_0 block after configuration by setting the
...
{
	// Set teh duty cycle by writing to the COMPA field (Bits 15 to 0)
	// in the PWM0CMPA register. When the counter matches the value in this register, 
	// the PWM signal will be driven high
	PWM0->_0_CMPA = (duty_cycle - 1);
}
